...
def receive(pp, addr_pk, addr_sk, ledger: Ledger):
    """
    inputs:
...
def verify_cm_and_sn(sk_enc, addr_pk, a_pk, a_sk, ledger: Ledger, Ciphertext, cm):
    (v, p, r, s) = D_enc(sk_enc, Ciphertext)
    k = comm_r(r, a_pk, p)
    sn = prf_sn(a_sk, p)
    if (cm == comm_s(v, k)) and ledger.verify_sn_inclusion(sn):
